...
/**
 * <a href="https://blog.csdn.net/lnktoking/article/details/79169011">使用说明</a>
 *
 * @version 20180827
 * @描述：时间日期操作
 * @Author linnankun
 * @timer 2015年2月11日 上午10:52:51
 */
...
public abstract class DateUtil {
    ...
    /**
     * 添加更多支持日期字符串格式
     * 默认支持：yyyyMMdd[ HHmmss]、yyyy-MM-dd[ HH:mm:ss]、yyyy/MM/dd[ HH:mm:ss]
     */
...
//		patternList.add("dd-MM-yyyy");
    ...
    public static class DateTime extends Date {
        ...
        private DateTime() {
            ...
            workdayConfig = DateUtil.workdayConfig();
        }
         ...
         * @param field 精确度
         * @param value 值
         *              <br/>{@link Calendar#SECOND}
         *              <br/>{@link Calendar#MINUTE}
         *              <br/>{@link Calendar#HOUR}
         *              <br/>{@link Calendar#DATE}
         *              <br/>{@link Calendar#MONTH}
         *              <br/>{@link Calendar#YEAR}
         * @return
         */
        ...
        public DateTime addWorkdays(int days) {
            DateTime date = DateUtil.date(this);
            if (days > 0) {
        ...
        public int compareWorkdays(Date date) {
            ...
            for (int i = 1; i <= days; i++) {
                if (DateUtil.date(start).addDays(i).isWorkday()) {
                    count++;
                }
            }
            return count;
    ...
    public static class WorkdayConfig implements Cloneable {
        ...
        public WorkdayConfig setWorkdays(List<Date> dates) {
            List<String> list = dates.stream().map(date -> {
                return DateUtil.date(date).toString("yyyy-MM-dd");
        ...
        public boolean isHoliday(Date date) {
            String d = DateUtil.date(date).toString("yyyy-MM-dd");
    ...
    //    System.out.println("--------------------");
    //    //支持多种日期字符串解析
    //    DateTime date1 = DateUtil.parseDate("2018-01-22");
    //    System.out.println(date1);
    //    date1 = DateUtil.parseDate("20180122102830");
    //    System.out.println(date1);
    //    date1 = DateUtil.parseDate("20180122");
    //    System.out.println(date1);
    //    date1 = DateUtil.parseDate("2018-01-22 10:28:30");
    //    System.out.println(date1);
    //
    //    System.out.println("------- 日期运算 --------");
    //    DateTime date2 = DateUtil.date()
    //            .addYears(1).addMonths(1).addDays(1)
    //            .addHours(1).addMinute(10).addSecond(30);
    //    DateTime date3 = DateUtil.date(date1).hour(9).minute(30);
    //
    ...
    //
    //    System.out.println("这个月最后一天是：" + DateUtil.date().monthLastDate().toString("yyyy-MM-dd"));
    //    System.out.println("今天是" + DateUtil.date().getZhWeek());
    //    System.out.println("这个星期的星期一是：" + DateUtil.date().weekStart().toString("yyyy-MM-dd"));
    //    System.out.println("这个星期的星期日是：" + DateUtil.date().weekEnd().toString("yyyy-MM-dd"));
    //
    ...
    //
    //    System.out.println("日期2018-04-26五个工作日之后的日期是：" + DateUtil.parseDate("2018-04-26").addWorkdays(5).toString());
    //    System.out.println("日期2018-05-04五个工作日之前的日期是：" + DateUtil.parseDate("2018-05-04").addWorkdays(-5).toString());
    //    System.out.println("2018-04-26与2018-05-04之间的工作日有：" + DateUtil.parseDate("2018-05-04").compareWorkdays(DateUtil.parseDate("2018-04-26")));
    //
    //    //设置大小周/单双周
    //    WorkdayConfig workdayConfig = DateUtil.getWorkdayConfig();
    //    workdayConfig.setWeekDayOff(
    //            new Integer[]{Calendar.SUNDAY},     //单周只休星期天
    //            new Integer[]{Calendar.SATURDAY, Calendar.SUNDAY});     //双周休两天
    //
    //    System.out.println("2018-05-05是休息日吗：" + DateUtil.parseDate("2018-05-05").workdayConfig(workdayConfig).isDayOff());
    //    System.out.println("2018-05-12是休息日吗：" + DateUtil.parseDate("2018-05-12").workdayConfig(workdayConfig).isDayOff());
    //
    //    DateUtil.workdayConfig().addMonthDayOff(5, new Integer[]{29, 30, 31});
    //    DateTime date = DateUtil.date(2018, 5, 1);
    //    System.out.printf("5月份上班天数：%s天，上班时间安排：\n", date.compareWorkdays(DateUtil.date(2018, 5, 31)));
    //    for (int i = 1; i <= 31; i++) {
    //        date.day(i);
    ...
    public static void main(String[] args) {
        ...
        List<Date> workdays = new ArrayList<>();
        List<Date> holidays = new ArrayList<>();
        List<String> workday = Arrays.asList(
                "2019-02-02",
                "2019-02-03",
                "2019-04-28",
                "2019-05-05",
        ...
        workday.forEach(date -> workdays.add(DateUtil.parseDate(date)));

        List<String> holiday = Arrays.asList(
                "2019-01-01",
                "2019-02-04",
                "2019-02-05",
                "2019-02-06",
        ...
        holiday.forEach(date -> holidays.add(DateUtil.parseDate(date)));
        ...
        workdayConfig.setWeekDayOff(
                ...
                //双周休两天
                new Integer[]{Calendar.SATURDAY, Calendar.SUNDAY});

        //DateTime dateTime = DateUtil.parseDate("2018-06-08");
        //DateTime dateTime1 = dateTime.workdayConfig(workdayConfig);
        //boolean dayOff = dateTime1.isDayOff();
        //System.out.println("2019-06-08是休息日吗：" + DateUtil.parseDate("2019-06-08").workdayConfig(workdayConfig).isDayOff());
        //System.out.println("2019-06-11是休息日吗：" + DateUtil.parseDate("2019-06-11").workdayConfig(workdayConfig).isDayOff());
