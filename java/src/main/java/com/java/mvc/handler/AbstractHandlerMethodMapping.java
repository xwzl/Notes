...
public abstract class AbstractHandlerMethodMapping<T> extends AbstractHandlerMapping implements InitializingBean {
    ...
    protected void processCandidateBean(String beanName) {
         ...
         *  <2.1 >{@link RequestMappingHandlerMapping#isHandler}抽象方法，判断 Bean 的类型是否为处理器。
         */
        if (beanType != null && isHandler(beanType)) {
            // <2.2>调用 #detectHandlerMethods(Object handler) 方法， 扫描处理器的方法们。详细解析，见
    ...
    protected void detectHandlerMethods(Object handler) {
        // <1> 获得处理器类型.这里也分成两种情况，和 「3.3 createHandlerMethod」 的一样，就不重复赘述了。
        ...
        if (handlerType != null) {
             ...
             * {@link com.java.mvc.handler.ClassUtils#getUserClass(Class)}
             */
            final Class<?> userType = ClassUtils.getUserClass(handlerType);
            // <3> 获得匹配的方法的集合
            Map<Method, T> methods = MethodIntrospector.selectMethods(userType,
                    /**
                     * todo  getMappingForMethod 抽象方法，获得方法的 Mapping 信息。子类实现,详细解析，见 「5.4 getMappingForMethod」 。
                     * {@link RequestMappingHandlerMapping#getMappingForMethod}
                     */
            ...
            // 这样，Mapping 和 handler + method ，就成功注册到 mappingRegistry 中。那么，可能比较重要的问题是，究竟 Mapping 是什么 ？
            // 实际上，在 RequestMappingHandlerMapping 中，就是 @RequestMapping 解析成的 RequestMappingInfo 对象。淡定淡定，继续细细
            // 往下看。
    ...
    protected HandlerMethod createHandlerMethod(Object handler, Method method) {
        ...
        if (handler instanceof String) {
            ...
            /**
             * todo 到了此处，我们已经可以觉察，实际上，HandlerMethod 是 handler + method 的组合，一个对象的某个方法。
             * {@link com.java.mvc.handler.HandlerMethod#HandlerMethod(Object, Method)}
             */
    ...
    protected HandlerMethod getHandlerInternal(HttpServletRequest request) throws Exception {
        ...
        this.mappingRegistry.acquireReadLock();
        try {
            // <3> todo 获得 HandlerMethod 对象
            HandlerMethod handlerMethod = lookupHandlerMethod(lookupPath, request);
            /**
             * <4> 进一步，获得 HandlerMethod 对象
             * {@link com.java.mvc.handler.HandlerMethod#createWithResolvedBean()}
             */
    ...
    protected HandlerMethod lookupHandlerMethod(String lookupPath, HttpServletRequest request) throws Exception {
        ...
        // 其中，Match 是 AbstractHandlerMethodMapping 的内部类，
        List<Match> matches = new ArrayList<>();
        // <1.1> 优先，基于直接 URL 的 Mapping 们，进行匹配
        List<T> directPathMatches = this.mappingRegistry.getMappingsByUrl(lookupPath);
        if (directPathMatches != null) {
            // todo 将当前请求和注册表中的 Mapping 进行匹配。若匹配成功，则生成 Mapping 记录，添加到 matches 中。
    ...
    private void addMatchingMappings(Collection<T> mappings, List<Match> matches, HttpServletRequest request) {
        ...
        for (T mapping : mappings) {
            /**
             * <1> 执行匹配,如果匹配，则返回的结果非空。
             * todo {@link org.springframework.web.servlet.mvc.method.RequestMappingInfoHandlerMapping#getMatchingMapping}
             *
    ...
    protected void handleMatch(T mapping, String lookupPath, HttpServletRequest request) {
         ...
         * 详细解析，见 「5. RequestMappingInfoHandlerMapping」 中。
         */
    ...
    protected HandlerMethod handleNoMatch(Set<T> mappings, String lookupPath, HttpServletRequest request)
            throws Exception {
        /**
         *  目前是空实现，在 RequestMappingInfoHandlerMapping 中，会被重写。详细解析，见
         *  「5. RequestMappingInfoHandlerMapping」 中。
         */
        return null;
    }
    ...
    class MappingRegistry {
        ...
        public void register(T mapping, Object handler, Method method) {
            ...
            try {
                ...
                // <3.1>todo 获得 mapping 对应的普通 URL 数组
                ...
                // 关于 MappingRegistration 类，详细解析，见 「3.4 MappingRegistration」 。
                //this.registry.put(mapping, new MappingRegistration<>(mapping, handlerMethod));
                this.registry.put(mapping, null);
            } finally {
                // <7> 释放写锁
    ...
    private class MatchComparator implements Comparator<Match> {
        ...
        /**
         * 重点在于 #compare(Match match1, Match match2) 方法的实现。通过 mapping 属性，即 Mapping 对象。
         * 详细解析，见 「4.2 RequestMappingInfo」
         */
