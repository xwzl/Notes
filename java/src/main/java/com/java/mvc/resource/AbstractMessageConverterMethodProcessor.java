...
public abstract class AbstractMessageConverterMethodProcessor extends AbstractMessageConverterMethodArgumentResolver
	...
	protected <T> void writeWithMessageConverters(@Nullable T value, MethodParameter returnType,
		...
		// boyd 对应真正的返回实体
		// valueType 对应真正的返回实体的类型
		// targetType 返回实体对应的泛型参数
		Object body;
		Class<?> valueType;
		Type targetType;
		...
		// <2> 是否为 Resource 类型。因为我们实际使用时，不会返回 Resource 类型，所以暂时无视这个块逻辑。
		...
		if (contentType != null && contentType.isConcrete()) {
			//目前调试下来，<3.1> 处，返回的都是 null 。所以此处的逻辑，可以无视。暂时不知道，什么情况下返回的是非 null 。
			...
			// <3.2.2> todo 获得可产生的 MediaType 数组
			...
			// <3.2.4> todo 通过 acceptableTypes 来比对，将符合的 producibleType 添加到 mediaTypesToUse 结果数组中
			...
			// <3.2.7> todo 选择其中一个最匹配的，主要考虑不包含通配符的。例如 application/json;q=0.8 。
			for (MediaType mediaType : mediaTypesToUse) {
				if (mediaType.isConcrete()) {
					selectedMediaType = mediaType;
					break;
				} else if (mediaType.equals(MediaType.ALL) || mediaType.equals(ALL_APPLICATION_MEDIA_TYPES)) {
		...
		if (selectedMediaType != null) {
			...
			for (HttpMessageConverter<?> converter : this.messageConverters) {
				...
				if (genericConverter != null ?
					...
					// <5.1> todo 如果有 RequestResponseBodyAdvice ，则可以对返回的结果，做修改。
					//  关于 RequestResponseBodyAdvice 的定义，
	...
	protected List<MediaType> getProducibleMediaTypes(
		...
		if (!CollectionUtils.isEmpty(mediaTypes)) {
			...
			// <x> todo 如果 allSupportedMediaTypes 非空，则遍历 HttpMessageConverter 数组，进行类型匹配
