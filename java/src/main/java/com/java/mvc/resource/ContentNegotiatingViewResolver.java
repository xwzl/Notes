 ...
 *
 * <p>For controller, if the request path is {@code /view.html}, this view resolver will look for a view
...
public class ContentNegotiatingViewResolver extends WebApplicationObjectSupport
    ...
    protected void initServletContext(ServletContext servletContext) {
        ...
        // todo 默认情况下，走的是这段逻辑。所以此时 viewResolvers 会有 BeanNameViewResolver、ThymeleafViewResolver、
    ...
    public View resolveViewName(String viewName, Locale locale) throws Exception {
        ...
        // <1> 获得 MediaType 数组 详细解析，见 「4.4.1 getMediaTypes」 。
        List<MediaType> requestedMediaTypes = getMediaTypes(((ServletRequestAttributes) attrs).getRequest());
        if (requestedMediaTypes != null) {
            // <2.1> 获得匹配的 View 数组 见 「4.4.2 getCandidateViews」 。
            List<View> candidateViews = getCandidateViews(viewName, locale, requestedMediaTypes);
            // <2.2> 筛选最匹配的 View 对象 见 「4.4.3 getBestView」
    ...
    protected List<MediaType> getMediaTypes(HttpServletRequest request) {
        ...
        /**
         * 虽然逻辑比较长，但是在{@link com.java.mvc.component.adapter.ReturnValue} 中的 「5.4.1 HandlerMethodReturnValueHandler」
         * 中，已经看过类似的 MediaType 的匹配逻辑，所以就不重复赘述。
         */
        try {
    ...
    private List<View> getCandidateViews(String viewName, Locale locale, List<MediaType> requestedMediaTypes)
        ...
        if (this.viewResolvers != null) {
            ...
            for (ViewResolver viewResolver : this.viewResolvers) {
                ...
                // 当然，默认情况下，这个逻辑，我们可以无视，因为在 <1.3.2> 处，我们在默认情况下，并未配置 MediaType 对应的拓展后缀。
                View view = viewResolver.resolveViewName(viewName, locale);
                if (view != null) {
                    candidateViews.add(view);
                }
                // todo <1.3> 情况二，带有文拓展后缀的方式，获得 View 对象，添加到 candidateViews 中
                ...
                for (MediaType requestedMediaType : requestedMediaTypes) {
                    ...
                    for (String extension : extensions) {
                        // todo <1.3.4> 带有文拓展后缀的方式，获得 View 对象，添加到 candidateViews 中
