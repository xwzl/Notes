...
import com.java.mvc.component.other.MultipartComponent;
import com.java.mvc.component.other.RequestToViewNameTranslatorComponent;
 ...
 * {@link HandlerExceptionResolver}, for controller mapping certain exceptions to error pages.
 * Default are
...
public class DispatcherServlet extends FrameworkServlet {
     ...
     * set by views (for controller, JSTL's).
     * <p>Default is "true", which is strongly recommended. Views should not rely on request attributes
     * having been set by (dynamic) includes. This allows JSP views rendered by an included controller
     * to use any model attributes, even with the same names as in the main JSP, without causing side
     * effects. Only turn this off for special needs, for controller to deliberately allow main JSPs to
    ...
    private void initHandlerExceptionResolvers(ApplicationContext context) {
        ...
        // 情况一，自动扫描 HandlerExceptionResolver 类型的 Bean 们 ,默认情况下 detectAllHandlerExceptionResolvers 为true
    ...
    protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            try {
                // TODO 检查是否是上传请求。如果是，则封装成 MultipartHttpServletRequest 对象。
                processedRequest = checkMultipart(request);
                /**
                 * 详细解析，见{@link MultipartComponent}
                 */
                multipartRequestParsed = (processedRequest != request);

                // Determine handler for the current request.
                // <3> 获得请求对应的 HandlerExecutionChain 对象,返回请求对应的是 HandlerExecutionChain 对象，
                 ...
                 *  <4.1> TODO 详情见{@link com.java.mvc.component.adapter.HandlerAdapterComponent}
                 */
                ...
                /**
                 * <5> 拦截器的前置处理，详情见{@link com.java.mvc.component.mapping.HandlerInterceptorComponent}
                 */
                ...
                /**
                 *  <8> ，当无视图的情况下，设置默认视图。
                 *  详情见{@link RequestToViewNameTranslatorComponent}
                 */
                applyDefaultViewName(processedRequest, mv);
                /**
                 * <9> TODO 后置处理 拦截器，拦截器的后置处理，即调用
                 * {@link com.java.mvc.component.mapping.HandlerInterceptorComponent}
                 */
                mappedHandler.applyPostHandle(processedRequest, response, mv);
            } catch (Exception ex) {
                // <10> todo 记录异常。注意，此处仅仅记录，不会抛出异常，而是统一交给 <11> 处理
            ...
            // <11> todo 处理正常和异常的请求调用结果。注意，正常的、异常的，都会进行处理。 ======
            processDispatchResult(processedRequest, response, mappedHandler, mv, dispatchException);
        } catch (Exception ex) {
            // <12> todo 已完成 拦截器 拦截器的已完成处理，即调用
    ...
    private void processDispatchResult(HttpServletRequest request, HttpServletResponse response,
        ...
        if (mv != null && !mv.wasCleared()) {
            ...
            // <3.2> 清理请求中的错误消息属性
            if (errorView) {
                // 清理请求中的错误消息属性。为什么会有这一步呢？
                // 在#processHandlerException方法中，会调用 WebUtils#exposeErrorRequestAttributes
                // 方法，设置请求中的错误消息属性。
    ...
    protected HandlerExecutionChain getHandler(HttpServletRequest request) throws Exception {
        if (this.handlerMappings != null) {
            ...
            for (HandlerMapping mapping : this.handlerMappings) {
                ...
                if (handler != null) {
                    /**
                     * 详情见{link {@link com.java.mvc.component.mapping.HandlerInterceptorComponent}
                     */
                    return handler;
                }
            }
        ...
        if (this.handlerAdapters != null) {
            ...
            for (HandlerAdapter adapter : this.handlerAdapters) {
                ...
                if (adapter.supports(handler)) {
                    /**
                     * 如果支持，则返回,详情见{@link com.java.mvc.component.adapter.HandlerAdapterComponent}
                     */
                    return adapter;
                }
            }
     ...
     *                 (for controller, if multipart resolution failed)
    ...
    protected void render(ModelAndView mv, HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try {
            ...
            /**
             * <4> 渲染页面 详情见{@link ViewResolverComponent}
             */
