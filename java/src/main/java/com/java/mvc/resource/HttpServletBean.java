...
import com.java.mvc.component.WebApplicationContext;
...
public abstract class HttpServletBean extends HttpServlet implements EnvironmentCapable, EnvironmentAware {
     ...
     */
    public final Set<String> requiredProperties = new HashSet<>(4);
    ...
    public final void init() throws ServletException {
        ...
        // <1> 解析 <init-param /> 标签，封装到 PropertyValues pvs 中其中，ServletConfigPropertyValues 是 HttpServletBean
        // 的私有静态类，继承 MutablePropertyValues 类，ServletConfig 的 PropertyValues 封装实现类。
        ...
        if (!pvs.isEmpty()) {
            try {
                ...
                // <2.3> 空实现，留给子类覆盖,然而实际上，子类暂时木有任何实现。
                initBeanWrapper(bw);
                // <2.4> 以 Spring 的方式来将 pvs 注入到该 BeanWrapper 对象中
                // 此处有配置了 contextConfigLocation 属性，那么通过 <2.4> 处的逻辑，会反射设置到 FrameworkServlet.contextConfigLocation 属性。
        ...
        /**
         * <3> 子类来实现，实现自定义的初始化逻辑。目前，有具体的代码实现。目前，FrameworkServlet 实现类该方法。
         * 详情{@link WebApplicationContext#frameworkServlet()} 或者{@link FrameworkServlet#initServletBean()}
         */
        initServletBean();
    }
