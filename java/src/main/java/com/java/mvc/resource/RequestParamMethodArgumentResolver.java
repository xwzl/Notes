...
public class RequestParamMethodArgumentResolver extends AbstractNamedValueMethodArgumentResolver
    ...
    public boolean supportsParameter(MethodParameter parameter) {
        ...
        if (parameter.hasParameterAnnotation(RequestParam.class)) {
            // <3> 如果是 Map 类型，则 @RequestParam 注解必须要有 name 属性是不是感觉有几分灵异？
            // 答案在 「6. RequestParamMapMethodArgumentResolver」 一起讲。
            ...
            // <1> 如果 Multipart 参数。则返回 true ，表示支持,上传文件相关的类型。
            if (MultipartResolutionDelegate.isMultipartArgument(parameter)) {
                return true;
                // <2> 如果开启 useDefaultResolution 功能，则判断是否为普通类型
            } else if (this.useDefaultResolution) {
                /**
                 * todo 那么 useDefaultResolution 到底是怎么样的赋值呢？
                 * {@link RequestMappingHandlerAdapter#getDefaultArgumentResolvers}
                 *
                 * 我们可以看到有两个 RequestParamMethodArgumentResolver 对象，前者 useDefaultResolution 为 false ，
                 * 后者为 useDefaultResolution 为 true 。什么意思呢？优先将待有 @RequestParam 注解的请求参数给第一个
                 * RequestParamMethodArgumentResolver 对象；其次，给中间省略的一大片参数解析器试试能不能解析；最后，使用
                 * 第二个 RequestParamMethodArgumentResolver 兜底，处理剩余的情况。
                 */
