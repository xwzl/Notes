...
public abstract class BeanFactoryUtils {
    ...
    /**
     * {@link #transformedBeanNameCache}缓存 {@link #transformedBeanName(String)} 已经转换好的结果。
     * <p>
     * 去除 FactoryBean 的修饰符 &
     * <p>
     ...
     *
     * <ul>
     *     <li>际上，逻辑比较简单，就是去除传入 name 参数的 "&" 的前缀。</li>
     *     <li>transformedBeanNameCache 集合的存在，是为了缓存转换后的结果。下次再获取相同的 name 时，直接返回缓存中的结果即可。</li>
     * </ul>
     * 小知识补充。假设配置了一个 FactoryBean 的名字为 "abc" ，那么获取 FactoryBean 创建的 Bean 时，使用 "abc" ，如果获取 FactoryBean 本身，
     * 使用 "$abc" 。另外，&定义在 BeanFactory.FACTORY_BEAN_PREFIX = "&" 上。
     *
