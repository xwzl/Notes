...
import com.java.spring.bean.InitBean;
import com.java.spring.util.AutowireUtils;
import com.java.spring.util.ImplicitlyAppearedSingletonException;
import com.java.spring.xml.dependcy.DisposableBeanAdapter;
...
public abstract class AbstractAutowireCapableBeanFactory extends AbstractBeanFactory
     ...
     * Class objects: for controller, String. Default is none.
     */
     ...
     * (for controller, through AOP auto-proxying).
     ...
     * for controller, String. Default is none.
     */
        ...
        if (bd.getResolvedAutowireMode() == AUTOWIRE_CONSTRUCTOR) {
            ...
            if (System.getSecurityManager() != null) {
                bean = AccessController.doPrivileged((PrivilegedAction<Object>) () ->
                                getInstantiationStrategy().instantiate(null, null, parent),
                        getAccessControlContext());
            } else {
                bean = getInstantiationStrategy().instantiate(null, null, parent);
            }
    ...
    public Object createBean(String beanName, RootBeanDefinition mbd, @Nullable Object[] args)
        ...
        // <1> 确保此时的 bean 已经被解析了,主要是解析 bean definition 的 class 类，并将已经解析的 Class 存储在 bean
        // definition 中以供后面使用。
        ...
        try {
             ...
             * {@link AbstractBeanDefinition#prepareMethodOverrides()}
             */
        ...
        try {
             ...
             *  <3> 实例化的前置处理是给 BeanPostProcessors 后置处理器返回一个代理对象的机会。其实在调用该方法之前 Spring
             *  一直都没有创建 bean ，那么这里返回一个 bean 的代理类有什么作用呢？作用体现在后面的 if 判断，
        ...
        try {
            // <4> 如果没有代理对象，就只能走常规的路线进行 bean 的创建了,创建 Bean 对象，进入方法内部
     ...
     */
    public Object doCreateBean(final String beanName, final RootBeanDefinition mbd, final @Nullable Object[] args)
            throws BeanCreationException {
                    ...
                    if (!actualDependentBeans.isEmpty()) {
                        throw new BeanCurrentlyInCreationException(beanName,
                                "Bean with name '" + beanName + "' has been injected into other beans [" +
                                        ...
                                        "'getBeanNamesOfType' with the 'allowEagerInit' flag turned off, for controller.");
                    }
                }
            }
        }
    ...
    protected Class<?> getTypeForFactoryBean(String beanName, RootBeanDefinition mbd) {
        ...
        if (factoryBeanName != null) {
            if (factoryMethodName != null) {
                ...
                BeanDefinition fbDef = (BeanDefinition) getBeanDefinition(factoryBeanName);
    ...
    protected void applyMergedBeanDefinitionPostProcessors(RootBeanDefinition mbd, Class<?> beanType, String beanName) {
        for (BeanPostProcessor bp : getBeanPostProcessors()) {
            if (bp instanceof MergedBeanDefinitionPostProcessor) {
                ...
                bdp.postProcessMergedBeanDefinition(null, beanType, beanName);
            }
        }
    }
    ...
    public BeanWrapper createBeanInstance(String beanName, RootBeanDefinition mbd, @Nullable Object[] args) {
        ...
        if (instanceSupplier != null) {
            /**
             * 那么 Supplier 是什么呢？在这之前也没有提到过这个字段。{@link Supplier} 接口.
             * <ul>
             *     <li>Supplier 接口仅有一个功能性的 #get() 方法，该方法会返回一个 <T> 类型的对象，有点儿类似工厂方法。</li>
             *     <li>这个接口有什么作用？用于指定创建 bean 的回调。如果我们设置了这样的回调，那么其他的构造器或者工厂方法都会没有用。</li>
             * </ul>
             * 在什么设置该 Supplier 参数呢？Spring 提供了相应的 setter 方法，如下：
             * {@link AbstractBeanDefinition#setInstanceSupplier(Supplier)}
             *
             * 构造 BeanDefinition 对象的时候，RootBeanDefinition了 instanceSupplier 该值，代码（以 RootBeanDefinition 为例）如下:
             * {@link RootBeanDefinition#RootBeanDefinition(Class, String, Supplier)}
             * 如果设置了 instanceSupplier 属性，则可以调用{@link #obtainFromSupplier}方法，完成 Bean 的初始化。
             */
        ...
        // <4.2> 有参数时，又没获取到构造方法，则只能调用无参构造方法来创建实例了(兜底方法)第四种创建方式
        return instantiateBean(beanName, mbd);
    }
     ...
     * @since 5.0
     */
    public BeanWrapper obtainFromSupplier(Supplier<?> instanceSupplier, String beanName) {
        Object instance;
        // 获得原创建的 Bean 的对象名
        ...
        try {
            ...
            if (System.getSecurityManager() != null) {
                beanInstance = AccessController.doPrivileged((PrivilegedAction<Object>) () ->
                                ...
                                getInstantiationStrategy().instantiate(null, beanName, parent),
                        getAccessControlContext());
            } else {
                // 获得 InstantiationStrategy 对象，并使用它，创建 Bean 对象
                beanInstance = getInstantiationStrategy().instantiate(null, beanName, parent);
            }
    ...
    public void populateBean(String beanName, RootBeanDefinition mbd, @Nullable BeanWrapper bw) {
        ...
        /**
         * <2> 自动注入 =========================================================================================================
         * {@link AbstractBeanDefinition#getResolvedAutowireMode()}
         */
    ...
    public void autowireByName(
        ...
        // <1> 对 Bean 对象中非简单属性
        // 该方法逻辑很简单，获取该 bean 的非简单属性。什么叫做非简单属性呢？就是类型为对象类型的属性，但是这里并不是将所有的对
        // 象类型都都会找到，比如 8 个原始类型，String 类型 ，Number类型、Date类型、URL类型、URI类型等都会被忽略。
        String[] propertyNames = unsatisfiedNonSimpleProperties(null, bw);
        // 遍历 propertyName 数组
        // 获取需要依赖注入的属性后，通过迭代、递归的方式初始化相关的 bean ，然后调用 #registerDependentBean(String beanName,
        // String dependentBeanName) 方法，完成注册依赖。
        for (String propertyName : propertyNames) {
            ...
            if (containsBean(propertyName)) {
                ...
                /**
                 * 属性依赖注入 进入{@link DefaultSingletonBeanRegistry#registerDependentBean(String, String)}
                 */
    ...
    public void autowireByType(
        ...
        // 方法，来注册依赖。不过相对于 #autowireByName(...) 方法而言，根据类型寻找相匹配的 bean 过程比较复杂
        // 获取 TypeConverter 实例
        ...
        for (String propertyName : propertyNames) {
            try {
                ...
                if (Object.class != pd.getPropertyType()) {
                    ...
                    /**
                     * {@link com.java.spring.util.DefaultListableBeanFactory#resolveDependency(DependencyDescriptor, String, Set, TypeConverter)}
                     */
    ...
    protected void checkDependencies(
            String beanName, RootBeanDefinition mbd, PropertyDescriptor[] pds, @Nullable PropertyValues pvs)
    ...
    public Object convertForProperty(
        ...
        if (converter instanceof BeanWrapperImpl) {
            ...
            MethodParameter methodParam = BeanUtils.getWriteMethodParameter(pd);
            /**
             * 否则，调用 TypeConverter 的 #convertIfNecessary方法，进行类型转换。TypeConverter
             * 是定义类型转换方法的接口，通常情况下与 PropertyEditorRegistry 配合使用实现类型转换。
             * convertIfNecessary方法的实现者有两个：DataBinder 和 TypeConverterSupport 类。
             * <ul>
             *     <li>DataBinder 主要用于参数绑定（熟悉 Spring MVC 的都应该知道这个类）</li>
             *     <li>TypeConverterSupport 则是 TypeConverter 的基本实现，使用的是 typeConverterDelegate 委托者。</li>
             * </ul>
             * 所以这里我们只需要关注{@link TypeConverterSupport#convertIfNecessary}方法================
             */
     ...
     */
    public void invokeInitMethods(String beanName, final Object bean, @Nullable RootBeanDefinition mbd)
            throws Throwable {
     ...
     * object obtained from FactoryBeans (for controller, to auto-proxy them).
     *
     * @see #applyBeanPostProcessorsAfterInitialization
     */
    @Override
