...
import com.java.spring.ioc.ParseBeanTag;
 ...
 * {@link BeanDefinitionReaderUtils BeanDefinitionParsers} or
...
public class BeanDefinitionParserDelegate {
    ...
    public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
        ...
        String beanName = id;
        // <3.2> 如果 id 为空，但是 aliases 不空，则 beanName 为 aliases 的第一个元素
        ...
        // <4> 解析属性，构造 AbstractBeanDefinition 对象 进入方法内部
        AbstractBeanDefinition beanDefinition = parseBeanDefinitionElement(ele, beanName, containingBean);
        if (beanDefinition != null) {
            // <3.3> 如果两者都为空，则根据默认规则来设置 beanName 。因为默认规则不是本文的重点，所以暂时省略。
    ...
    public AbstractBeanDefinition parseBeanDefinitionElement(
        ...
        try {
            // 创建用于承载属性的 AbstractBeanDefinition 实例 进入
            ...
            parseConstructorArgElements(ele, (BeanDefinition) bd);
            // 解析 property 子元素 <property />
            parsePropertyElements(ele, (BeanDefinition) bd);
     ...
     * 返回{@link BeanDefinitionParserDelegate#parseBeanDefinitionElement(Element, String, BeanDefinition)}
     *
    ...
    /**
     * meta 所声明的 key 并不会在 Bean 中体现，只是一个额外的声明，当我们需要使用里面的信息时，通过调用
     * BeanDefinition 的 #getAttribute(String name) 方法来获取。该子元素的解析过程，
     * <p>
     * 解析过程较为简单，获取相应的 key - value 构建 BeanMetadataAttribute 对象，然后调用 BeanMetadataAttributeAccessor#
     * addMetadataAttribute(BeanMetadataAttribute) 方法，添加 BeanMetadataAttribute 加入到 AbstractBeanDefinition 中。
     * <p>
     * 友情提示：
     * <p>
            ...
            try {
                ...
                if (index < 0) {
                    ...
                    try {
                        ...
                        // <2> 解析 ele 对应属性元素 进入方法 调用 #parsePropertyValue(Element ele, BeanDefinition bd, String propertyName) 方法，解析 constructor-arg 子元素，返回结果值
                        Object value = parsePropertyValue(ele, bd, null);
                        // <3> 根据解析的属性元素构造一个 ValueHolder 对象 根据解析的结果值，构造ConstructorArgumentValues.ValueHolder 实例对象，并将 type、name 设置到 ValueHolder 中
    ...
    public Object parsePropertyValue(Element ele, BeanDefinition bd, @Nullable String propertyName) {
        ...
        // <1> 查找子节点中，是否有 ref、value、list 等元素,提取 constructor-arg 的子元素、ref 属性值和 value 属性值，对其进行判断。
        //以下两种情况是不允许存在的：ref 和 value 属性同时存在 。存在 ref 或者 value 且又有子元素。
    ...
    public BeanDefinition parseCustomElement(Element ele, @Nullable BeanDefinition containingBd) {
        ...
        // <2> 根据 namespaceUri 获取相应的 Handler,根据 namespaceUri 获取相应的 Handler 对象。这个映射关系我们在
        // spring.handlers 中已经定义了，所以只需要找到该类，然后初始化返回。
        ...
        /**
         * 调用自定义的 Handler 处理,完成后返回 NamespaceHandler 对象，然后调用其 #parse(Element element,ParserContext parserContext)
         * 方法开始自定义标签的解析。{@link NamespaceHandlerSupport#parse(Element, ParserContext)}
         */
        return (BeanDefinition) handler.parse(ele, new ParserContext(this.readerContext, null, null));
    }
    ...
    /**
     * <1> 和 <2> 处，都是遍历，前者遍历的是属性( attributes )，后者遍历的是子节点( childNodes )，最终调用的都是
        ...
        if (namespaceUri != null && !isDefaultNamespace(namespaceUri)) {
            ...
            if (handler != null) {
                ...
                BeanDefinitionHolder decorated =
                        handler.decorate(node, originalDef, new ParserContext(this.readerContext, null, null));
                if (decorated != null) {
                    return decorated;
                }
    ...
    private BeanDefinitionHolder parseNestedCustomElement(Element ele, @Nullable BeanDefinition containingBd) {
        ...
        return new BeanDefinitionHolder((org.springframework.beans.factory.config.BeanDefinition) innerDefinition, id);
    }
