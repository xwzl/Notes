...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
    ...
    protected Object getSingleton(String beanName, boolean allowEarlyReference) {
        // 从单例缓冲中加载 bean，查看三个Map的含义
        Object singletonObject = this.singletonObjects.get(beanName);
        // 缓存中的 bean 为空，且当前 bean 正在创建
        //#isSingletonCurrentlyInCreation(String beanName) 方法，该方法用于判断该 beanName
        //对应的 Bean 是否在创建过程中，注意这个过程讲的是整个工厂中
    ...
    public Object getSingleton(String beanName, ObjectFactory<?> singletonFactory) {
        ...
        // 全局加锁
        synchronized (this.singletonObjects) {
            // <1> 从缓存中检查一遍,再次检查缓存是否已经加载过，如果已经加载了则直接返回，否则开始加载过程。
            ...
            if (singletonObject == null) {
                ...
                // <5> 加入缓存中,将结果记录并加入值缓存中，同时删除加载 bean 过程中所记录的一些辅助状态。
                if (newSingleton) {
                    //进入方法内部
                    addSingleton(beanName, singletonObject);
                }
     ...
     * (for controller, a DisposableBean adapter for a singleton that does not
