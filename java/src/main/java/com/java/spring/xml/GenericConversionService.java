...
public class GenericConversionService implements ConfigurableConversionService {
    ...
    /**
     * 所有 Converter 集合的封装对象，Converters 是 GenericConversionService 内部类，用于管理所
     * 有注册的转换器，其内部维护一个 Set 和 Map 的数据结构用于管理转换器，
     */
     ...
     * <ul>
     * <li>canConvert(sourceType, targetType) 方法，用于判 sourceType 能否转成 targetType 。</li>
     * <li>convert(source, targetType) 方法，用于将 source 转成转入的 TargetType 类型实例。</li>
     * </ul>
     * <p>
     * 这两类方法都是在 GenericConversionService 中实现。
     * <p>
     * 类 GenericConversionService 实现 ConfigurableConversionService 接口，而 ConfigurableConversionService 接口继承
     * ConversionService 和 ConverterRegistry。ConverterRegistry 提供了类型转换器的管理功能，他提供了四个 add 和一个 remove
     * 方法，支持注册/删除相应的类型转换器。
     * <p>
     * GenericConversionService 作为一个基础实现类，它即支持了不同类型之间的转换，也对各类型转换器进行管理，主要是通过一个 Map 类型的
     * converterCache 和一个内部类 Converters 。在上面已经分析了 GenericConversionService 执行类型转换的过程 #cover(...) 方法。
     * 下面我们就一个 addConverter(Converter<?, ?> converter) 方法，来看看它是如何完成转换器的注入的工作的。
     */
    @Override
    ...
    public Object convert(@Nullable Object source, @Nullable TypeDescriptor sourceType, TypeDescriptor targetType) {
        ...
        // <3> 获得对应的 GenericConverter 对象，用于转换类型======================
        GenericConverter converter = getConverter(sourceType, targetType);
    ...
    private static class Converters {
        ...
        public GenericConverter find(TypeDescriptor sourceType, TypeDescriptor targetType) {
            ...
            //在find中，会根据 sourceType 和 targetType 去查询 Converters 中维护的 Map 中是否包括支持的
            //注册类型。如果存在返回 GenericConverter ，如果没有存在返回 null 。
    ...
    private static class ConvertersForPair {
        ...
        /**
         * 其实内部就是维护一个 LinkedList 集合。他内部有两个方法：#add(GenericConverter converter) 和 getConverter
         * (TypeDescriptor sourceType, TypeDescriptor targetType)，实现较为简单，这里就不多介绍了。
         */
