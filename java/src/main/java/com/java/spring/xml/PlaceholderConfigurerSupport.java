 ...
 * <p>
 * In contrast to {@link org.springframework.beans.factory.config.PropertyOverrideConfigurer}, subclasses of this type allow
...
public abstract class PlaceholderConfigurerSupport extends PropertyResourceConfigurer
    ...
    public void doProcessProperties(ConfigurableListableBeanFactory beanFactoryToProcess,
        ...
        // <2.1> 创建 BeanDefinitionVisitor 对象根据 String 值解析策略 valueResolver 得到 BeanDefinitionVisitor
        // 实例。BeanDefinitionVisitor 是 BeanDefinition 的访问者，我们通过它可以实现对 BeanDefinition 内容的进行访问，
        ...
        for (String curName : beanNames) {
            ...
            if (!(curName.equals(this.beanName)
                ...
                // 2. 同一个 Spring 容器
                BeanDefinition bd = (BeanDefinition) beanFactoryToProcess.getBeanDefinition(curName);
                try {
                    /**
                     * 得到该容器的所有 BeanName，然后对其进行访问
                     * {@link com.java.spring.xml.BeanDefinitionVisitor#visitBeanDefinition(BeanDefinition)}访问
                     */
                    visitor.visitBeanDefinition((org.springframework.beans.factory.config.BeanDefinition) bd);
                } catch (Exception ex) {
