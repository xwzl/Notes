...
final class PostProcessorRegistrationDelegate {
    ...
    /**
     * 方法首先 beanFactory 获取注册到该 BeanFactory 中所有 BeanPostProcessor 类型的 beanName 数组，其实就是找所有实现了
     * BeanPostProcessor 接口的 bean ，然后迭代这些 bean ，将其按照 PriorityOrdered、Ordered、无序的顺序，添加至相应的 List
     * 集合中，最后依次调用 #sortPostProcessors方法来进行排序处理、 #registerBeanPostProcessors方法来完成注册。
     * <p>
     * 【排序】很简单，如果 beanFactory 为 DefaultListableBeanFactory ，则返回 BeanFactory 所依赖的比较器，否则反正默认的
     * 比较器(OrderComparator)，然后调用 List#sort(Comparator<? super E> c) 方法即可。
     */
            ...
            if (!(bean instanceof BeanPostProcessor) && !isInfrastructureBean(beanName) &&
                ...
                if (logger.isInfoEnabled()) {
                    logger.info("Bean '" + beanName + "' of type [" + bean.getClass().getName() +
                            ...
                            "(for controller: not eligible for auto-proxying)");
                }
            }
            return bean;
        }
